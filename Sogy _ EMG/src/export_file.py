...
def create_body(self, operator, driver, nopol, volumeBefore, volumeAfter, deltaVolume):
    ...
    # operator
    cell(40, 90, 'Operator')
    cell(95, 90, f' : {operator}')
    # driver
    cell(40, 100, 'Driver')
    cell(95, 100, f' : {driver}')
    # Nomor Polisi
    cell(40, 110, 'Nomor Polisi')
    cell(95, 110, f' : {nopol}')
    # Volume Sebelum
    cell(40, 120, 'Volume Sebelum')
    cell(95, 120, f' : {volumeBefore:.2f} L')

    # volume sesudah
    cell(40, 130, 'Volume Sesudah')
    cell(95, 130, f' : {volumeAfter:.2f} L')

    cell(40, 140, 'Delta Volume')
    cell(95, 140, f' : {delta:.2f} L')
...
def create_analytics_report(operator, driver, nopol, volumeBefore, volumeAfter, deltaVolume):
    ...
    create_body(pdf, operator, driver, nopol,
                volumeBefore, volumeAfter, deltaVolume)
    create_tembusan(pdf)
    ...
    with open(PATH + csv_fileName + '.csv', 'a', newline='') as csvfile:
        ...
        field_names = ['Timestamp', 'Operator', 'Driver', 'Nomor Polisi',
                       'Volume Sebelum', 'Volume Sesudah', 'Delta Volume']
        writer = csv.DictWriter(csvfile, fieldnames=field_names)

        if first == True:
            writer.writeheader()
        writer.writerow({'Timestamp': timestamp, 'Operator': operator, 'Driver': driver, 'Nomor Polisi': nopol,
                        'Volume Sebelum': volumeBefore, 'Volume Sesudah': volumeAfter, 'Delta Volume': deltaVolume})
